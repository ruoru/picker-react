...
class PickerColumn extends Component {
  ...
  adjustPosition(props) {
    const { data, defaultIndex, itemHeight, indicatorTop, indicatorHeight } =
      props || this.props;
  ...
  render() {
    ...
    return (
      <div
        className={classNames("weui-picker__group", className)}
      ...
      >
        ...
        <div className="weui-picker__content" style={styles} ref="content">
          {data.map((item, j) => {
            ...
            return (
              <div key={j} value={item[dataKeys.value]} className={itemCls}>
                {item[dataKeys.text]}
              </div>
            );
          })}
