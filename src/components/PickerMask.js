...
class PickerMask extends Component {
    ...
    render() {
        const {show, transparent, lang, onCancel, onOk, onMaskClick, children} = this.props;
        ...
        return show && (
            <div>
                <div
                    className={clz}
                    onClick={e => {if(onMaskClick) onMaskClick()}}
                >
                </div>
                <div className={cls}>
                    <div className="weui-picker__hd">
                        <a key='0' className="weui-picker__action" onClick={e => {if (onCancel) onCancel(e);} }> { lang.cancelBtn }</a>
                        <a key='1' className="weui-picker__action" onClick={e => {if (onOk) onOk(e);} }> { lang.okBtn }</a>
                    </div>
